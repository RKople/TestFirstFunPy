...
public class ScheduleActivity extends Activity {
    ...
    @Override protected void onCreate(Bundle b){
        ...
        Ui.header(root, this, "Automatisation", "Planning", "Le test 8 minutes reste inchangé. Les vraies séances utilisent maintenant une alarme Android de niveau AlarmClock, plus forte pour sortir la TV d’une longue veille.");
        ...
        feedback = Ui.muted(this,""); feedback.setMaxLines(10); actionsCard.addView(feedback,Ui.lp(-1,-2,this,7));
    ...
    private void refresh(){
        ...
        if(m==null){movieStatus.setText("Aucun film");movieStatus.setTextColor(Ui.MUTED);} else {
            ...
            label.append("  ·  Volume : ").append(AppState.FILM_VOLUME_PERCENT).append(" %");
    ...
    private void scheduleAt(long when, boolean test){
        ...
        if(test && when < now + 7*60_000L){feedback.setText("Le test complet a besoin d’environ 8 minutes pour reproduire le scénario validé.");return;}
        if(!test && when <= now+8*60_000L){feedback.setText("Pour une vraie séance, programme au moins 8 minutes à l’avance.");return;}
        ...
        long retryAt;
        if(test){
            // DO NOT change this path: it is the exact 8-minute scenario validated on the TV.
            wakeAt=now+2*60_000L;
            retryAt=now+5*60_000L;
        } else {
            long desired=when-AppState.preWakeMinutes(this)*60_000L;
        ...
        arm(am,when,directPi,id+":show-direct",test);

        PendingIntent targetWake = wakePending(id+":target-wake", id, movie, volume, when, when, sleepWhenDone, "play");
        arm(am,when,targetWake,id+":show-target",test);

        // The receiver-delivery delay and the physical panel wake delay are different things.
        // Real tests on this Philips show roughly 3 minutes between the wake command and visible panel-on.
        long visibleEstimate=wakeAt+3*60_000L;
        ...
        if(!test){
            try{
                ...
                o.put("title",movie.optString("title"));o.put("movie",movie.toString());o.put("durationMs",duration);o.put("endAt",endAt);o.put("volume",volume);
                ...
                o.put("alarmMode","alarm-clock");o.put("createdAt",now);
                a.put(o);AppState.setSchedules(this,a);
                LogStore.add(this,"Planning","Séance v1.8 ajoutée : "+movie.optString("title")+" · AlarmClock réveil "+time(wakeAt)+" · film "+time(when)+" · volume 37 %");
            }catch(Exception ignored){}
        } else {
            LogStore.add(this,"Test","Test complet validé armé · exact wake "+time(wakeAt)+" · retry "+time(retryAt)+" · film "+time(when)+" · volume 37 %");
        }

        StringBuilder result=new StringBuilder(test?"Test complet armé ✓":"Séance ajoutée ✓");
        if(!test)result.append("\nMode longue veille : AlarmClock critique ✓");
        ...
        result.append("\nVolume film : ").append(AppState.FILM_VOLUME_PERCENT).append(" %");
    ...
    /**
     * The short test deliberately keeps setExactAndAllowWhileIdle because that exact path is validated.
     * Real sessions use setAlarmClock through AlarmTools: Android treats it as the most critical public
     * alarm class and exits Doze before delivery.
     */
    private void arm(AlarmManager am,long when,PendingIntent pi,String showKey,boolean test){
        if(test){
            setExact(am,when,pi);
        }else{
            AlarmTools.setCritical(this,am,when,pi,AppState.requestCodeForId(showKey));
        }
    }
